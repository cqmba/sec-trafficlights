...
ALIAS_GATEWAY=ssas.gateway
ALIAS_TLC=ssas.tlc
ALIAS_EV=ssas.ev
ALIAS_FRONTEND=ssas.frontend
NAME_GATEWAY=gateway
NAME_TLC=tlc
NAME_EV=ev
NAME_FRONTEND=frontend
DOMAIN_TLC="CN=ssas.tlc"
DOMAIN_GATEWAY="CN=ssas.gateway"
DOMAIN_EV="CN=ssas.ev"
DOMAIN_FRONTEND="CN=ssas.frontend"
...
keytool -export -alias $ALIAS_TLC -storepass $TLC_STOREPASS -rfc -file ${NAME_TLC}.cer -keystore ${NAME_TLC}_keystore.jks
...
keytool -export -alias $ALIAS_GATEWAY -storepass $GATEWAY_STOREPASS -rfc -file ${NAME_GATEWAY}.cer -keystore ${NAME_GATEWAY}_keystore.jks
...
keytool -export -alias $ALIAS_EV -storepass $EV_STOREPASS -rfc -file ${NAME_EV}.cer -keystore ${NAME_EV}_keystore.jks
...
keytool -export -alias $ALIAS_FRONTEND -storepass $FRONTEND_STOREPASS -rfc -file ${NAME_FRONTEND}.cer -keystore ${NAME_FRONTEND}_keystore.jks
